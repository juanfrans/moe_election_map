var electionDataUrl = "data/ElectionData.csv";
var topojsonDataUrl = "data/World_50m_Updated.topojson";

function getData() {
  d3.queue()
    .defer(d3.csv, electionDataUrl)
    .defer(d3.json, topojsonDataUrl)
    .await(ready);
}

getData();

function ready(error, googleData, geoData) {
  if (error) {
    console.error("Error loading data:", error);
    throw error;
  }
  drawMap(googleData, geoData);
}

function drawMap(googleData, geoData) {
  ...
  var countries = topojson.feature(
    geoData,
    geoData.objects.World_50m_Updated,
  ).features; // This is crucial, make sure you are pointing to the right place in the file
  ...
  svg
    ...
    .on("click", function (d) {
      ...
      if (d3.select(this).attr("class") == "elections") {
        ...
        d3.select("#popup").select("#name").text(d.properties.NombreEsp);

        // Add description to popup
        d3.select("#popElectionType1").text(
          "Tipo de elección: " + d.properties.Tipo1,
        );
        if (d.properties.Fecha1_2) {
          d3.select("#popElectionDate1").text(
            d.properties.Fecha1_1 +
              ", " +
              d.properties.Ano1 +
              " (2da vuelta: " +
              d.properties.Fecha1_2 +
              ", " +
              d.properties.Ano1 +
              ")",
          );
        ...
        if (d.properties.Tipo2) {
          ...
          if (d.properties.Fecha2_2) {
            ...
            d3.select("#popElectionDate2").text(
              d.properties.Fecha2_1 + ", " + d.properties.Ano2,
            );
          }
        } else {
          d3.select("#popElectionType2").text("");
        ...
        if (d.properties.Tipo3) {
          ...
          if (d.properties.Fecha3_2) {
            ...
            d3.select("#popElectionDate3").text(
              d.properties.Fecha3_1 + ", " + d.properties.Ano3,
            );
          }
        } else {
          d3.select("#popElectionType3").text("");
        ...
        if (d.properties.Participacion) {
          d3.select("#popParticipation").text(
            "Participación en últimas elecciones: " +
              d.properties.Participacion,
          );
        } else {
          d3.select("#popParticipation").text("");
        }
  ...
  d3.select("#year").on("change", function (d) {
    ...
    d3.selectAll(".elections")
      .classed("elections", false)
    ...
    d3.selectAll(".country")
      ...
      .attr("class", function (d) {
        if (year == "allYears") {
          ...
          d.properties.Ano1 == year ||
          d.properties.Ano2 == year ||
          d.properties.Ano3 == year
        ) {
          return "elections";
        } else {
          return "country";
